...
Since this is not a compiler hack, its just a lot of macro magick there are a couple limitations, for example there may only be *CEXEPT_MAX_EXCEPTIONS* unique exceptions. *CEXEPT_MAX_EXCEPTIONS* defaults to 255, but it may be redefined before including to increase or decrese it.
